...
class LongTask(QThread):
    ...
    def bing(self, inpu):
        result = []
        r = requests.get(f"https://cn.bing.com/dict/search?q={inpu}").content.decode()
    ...
    # 爬取dict.cn
    def dictcn(self, inpu):
        result = []
        r = requests.get(f"http://dict.cn/{inpu}").content.decode()
        soup = BeautifulSoup(r, "html.parser")
        tags = soup.find(
            name=None, attrs={"class": "dict-basic-ul"}
        )  # 查找文本当中class为title的<p>元素
        ...
        def test(a, i):
            match a:
                case 0:
                    result = self.bing(i)
                case 1:
                    result = self.youdao(i)
                case 2:
                    result = self.dictcn(i)
            return result
...
class MainWindow(QMainWindow):  # 主界面
    ...
    def initUI(self):  # 界面初始化
        ...
        out = QAction("导出", self)
        out.setShortcut("Ctrl+S")
        file.addAction(out)
        out.triggered.connect(self.out)

        add = QAction("在文件后追加...", self)
        add.setShortcut("Ctrl+Shift+S")
        file.addAction(add)
    ...
    def select(self):  # 选取导入文件
        ...
        if types == ".txt":
            ...
            txt = []
            df = read_excel(directory[0], header=0)
    ...
    def out(self):  # 选择导出文件目录
        ...
        if directory == "":
            ...
            try:
                ...
                reply = QMessageBox.warning(
                    self, "提示", "操作无法完成，因为文件在其他程序中打开"
                )

    def add(self):
        directory = QFileDialog.getOpenFileName(
            self, "选取追加文件", "./", "EXcel文件 (*.xlsx)"
        )
        lol = self.lolabel.toPlainText()
        prout = self.prlabel.toPlainText()
        if directory[0] == "":
            ...
            try:
                ...
                row = df.shape[0]
                s4 = concat(
                    [df, DataFrame({"英文单词": list2, "中文释义": list4})],
                    ignore_index=True,
                )
                s4.to_excel(directory[0], index=False)
                fil = openpyxl.load_workbook(directory[0])
                sheet = fil.active
                for a in sheet:
                    for c in a:
                        c.alignment = openpyxl.styles.Alignment(wrapText=True)
